...
// public functions
function sizeOfText( $texte, $largeur=0 )
{   
    	$index    = 0;
	$nb_lines = 0;
	$loop     = TRUE;
...
function addClient( $ref )
{
	...
	$this->SetXY( $r1 + ($r2-$r1)/2 - 5, $y1+3 );
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(10,5, "INVOICE NO", 0, 0, "C");
	$this->SetXY( $r1 + ($r2-$r1)/2 - 5, $y1 + 9 );
	$this->SetFont( "Arial", "", 10);
...
function addReglement( $mode )
{
	...
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(10,4, "Name : ".$mode, 0, 0, "C");
...
function temporaire( $texte )
{
	...
	function total($tva){
		...
		$mid = $y1 + (($y2-$y1) / 2);
		$this->SetXY( $r1 + 16 , $y1+1 );
		$this->Cell(50, 4, "Grand Total :"." ".$tva, '', '', "C");
		$this->SetFont( "Arial", "", 10);
		$this->SetXY( $r1 + 16 , $y1+5 );
	}
	function credit($tva){
		...
		$mid = $y1 + (($y2-$y1) / 2);
		$this->SetXY( $r1 + 16 , $y1+1 );
		$this->Cell(40, 4, "Special Credit :"." ".$tva, '', '', "C");
		$this->SetFont( "Arial", "", 10);
		$this->SetXY( $r1 + 16 , $y1+5 );
	}
}
