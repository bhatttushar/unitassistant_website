...
class Reggie_canada_model extends CI_Model {
	...
	function add_to_ua($data){
	    ...
	    if ($id_newsletter > 0) {
	      ...
	      foreach ($prevData as $key => $value) {
	        ...
	        unset($prevDesignData['distribution_one']);
	        unset($prevDesignData['distribution_two']);
	        unset($prevDesignData['status_one']);
	        unset($prevDesignData['status_two']);
	        unset($prevDesignData['status_five']);
	        unset($prevDesignData['status_six']);
	        unset($prevDesignData['status_eight']);
	        unset($prevDesignData['consultant_two']);
	        unset($prevDesignData['birthday_one']);
	        unset($prevDesignData['last_one']);
	        unset($prevDesignData['consultant_two1']);
	        unset($prevDesignData['beatly_url']);

	      unset($prevDesignData['id_newsletter_design_info']);
	      unset($prevDesignData['id_newsletter']);

	      $designData['id_newsletter'] = $id_newsletter;
	      $prevDesignData['hidden_newsletter']= 'AC';
	      ...
	      foreach ($prevData as $key => $value) {
            ...
            unset($prevPackData['beatly_url_one']);
	      	...
	      	$previousData['point_value'] = '4';
			$packagingData['total_text_program7']= '1';
			$packagingData['canada_service']= '1';
			$packagingData['package']= 'N';
			
			$packagingData['digital_biz_link'] = 'www.unitassist.com/'. str_replace(' ', '', $name); 

	      	$total = (float)GetPackageValue('N', $data['unit_size']);
	      	if ($packagingData['total_text_program7'] == '1') {
                $total += (float)total_text_program7_y;
            }

            if ($packagingData['canada_service'] == 1) {
                $total += (float)canada_service;
            }
            
            $data['sub_total'] = $total;

            if ($data['add_to']=='3') { 
            	$prevPackData['cc_billing']= '1';
            	$total +=(float)UACC_BILLING;
            }
  	...
  	function get_ua_mail_data($id_newsletter){
  	...
  	function get_uacc_mail_data($id_cc_newsletter){
  		$this->db->select('cc_director_title, name, email, cell_number, dob, consultant_number, intouch_password, reffered_by, national_area, seminar_affiliation, cc_number, cc_code, cc_expir_date, cc_zip, package_pricing, cc_newsletter');
  	...
  	function add_to_email_records($id_newsletter=0, $id_cc_newsletter=0){
  		$data = array(
