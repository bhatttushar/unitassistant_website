            ...
            <div class="col-sm-12">
                <div class="box">
                    <div class="box-body">
                                    ...
                                    <?php 
                                        ...
                                        foreach ($data as $val) { ?>
                                            <tr class="odd gradeX">
                                                ...
                                                <td>
                                                    <a class="btn btn-info mb7" href="<?php echo base_url('assets/uploads/billing/'.$val['invoice_name']); ?>" title="See Invoice" download="<?php echo $val['invoice_name']; ?>">View Details</a>&nbsp;

                                                    <a class="btn btn-info mb7" href="<?php echo base_url('billing/edit-invoice/'.$val['id_invoice']); ?>" title="Edit Invoice">Edit Invoice</a>&nbsp;

                                                    <form action="<?php echo base_url('billing/delete-invoice-pdf'); ?>" method="post" class="inline">
                                                        <input type="hidden" name="id_invoice" value="<?php echo $val['id_invoice']; ?>">
                                                        <input class="btn btn-info mb7" onclick="return confirm('Are you sure you want to delete this invoice?')" data-id="<?php echo $val['id_invoice']; ?>"  type="submit" name="pdf_delete" value="Delete">
                                                    </form>
                                                   
                                                    <input class="btn btn-info custom-paid" data-id="<?php echo $val['id_invoice']; ?>"  type="button" name="submit_paid" value="Make Payment">&nbsp;
                                                     
                                                    <input class="btn btn-info custom-unpaid" data-id="<?php echo $val['id_invoice']; ?>"  type="button" name="submit_unpaid" value="Unpaid">&nbsp;
                                                    
                                                    <input class="btn btn-info custom-send" data-id="<?php echo $val['id_invoice']; ?>" data-news="<?php echo $val['id_newsletter']; ?>" type="button" name="send" value="Send">
                        ...
                        <div class="Abal">
                            <label>Account Balance: <?php echo '$'. (GetTotalDue($id_newsletter) - GetBalance($id_newsletter)); ?></label>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="form-group">
                ...
                <input type="hidden" name="id_invoice" id="unpaid_id_invoice" value="" >
          </div>
        </div>
      ...
      <div class="modal-body">
        <form method="post" action="" name="mail_details">
            <div class="form-group">
                ...
                <input type="hidden" name="id_invoice" id="send_id_invoice" value="" >
                <input type="hidden" name="id_news" id="send_id_news" value="" >
          </div>
        </form>
      </div>
...
$(function () {
    ...
    $('#send_data').click(function(e){
        ...
        $.ajax({
            type: 'POST',
            url: baseURL + 'billing/make-payment',
            data: {is_paid:1, id_invoice:id_invoice, amount:amount},
            cache: false,
    ...
    $('#send_unpiad_data').click(function(e){
        var id_invoice =  $('#unpaid_id_invoice').val();
        var amount =  $('#unpaid_amount').val();
        $.ajax({
            type: 'POST',
            url: baseURL + 'billing/unpaid',
            data: {is_unpaid:1, id_invoice:id_invoice, amount:amount},
            cache: false,
            success: function(data){  
                location.reload();
            }
    ...
    $('body').on('click','.custom-send',function(e){
        ...
        $('#send_id_invoice').val(id);
        $('#send_id_news').val(id_news);

        $.ajax({
            type: 'POST',
            url: baseURL + 'billing/get-mail-content',
            data: {is_mail_get : 1, id_newsletter:id_news, id_invoice:id},
    ...
    $('#send_mail_data').click(function(e){
        e.preventDefault();
        var id_invoice =  $('#send_id_invoice').val();
        var id_newsletter =  $('#send_id_news').val();
        var markup = $('.note-editable').html();
        $('#send_mail').modal('hide');
        $.ajax({
            type: 'POST',
            url: baseURL + 'billing/send-invoice-mail',
            data: {is_mail_send : 1, id_newsletter:id_newsletter, id_invoice:id_invoice, e_data:markup},
            beforeSend: function(data){
                $('.overlay_ajax').show();
            },
            success: function(data){  
